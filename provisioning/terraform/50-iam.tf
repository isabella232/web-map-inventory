...
resource "aws_iam_user" "bas-gitlab-ci-bas-web-map-inventory" {
  name = "bas-gitlab-ci-bas-web-map-inventory"
}
...
# Inline policy
#
# This resource implicitly depends on the 'aws_iam_user.bas-gitlab-ci-bas-web-map-inventory' resource
# This resource relies on the AWS Terraform provider being previously configured.
#
...
resource "aws_iam_user_policy" "bas-web-map-inventory-metadata-standards-production-management-policy" {
  name   = "bas-web-map-inventory-metadata-standards-production-management-policy"
  user   = "${aws_iam_user.bas-gitlab-ci-bas-web-map-inventory.name}"
